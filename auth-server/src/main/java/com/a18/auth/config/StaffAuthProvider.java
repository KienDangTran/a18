...
public class StaffAuthProvider implements AuthenticationProvider {
  ...
  /**
   * The password used to perform {@link PasswordEncoder#matches(CharSequence, String)} on when the user is not found to
   * avoid SEC-2056. This is necessary, because some {@link PasswordEncoder} implementations will short circuit if the
  ...
  private UserDetails retrieveUser(String username, StaffAuthenticationToken authentication) {
    ...
    try {
      ...
      if (loadedUser == null
          || !this.passwordEncoder.matches(String.valueOf(authentication.getCredentials()), loadedUser.getPassword())) {
